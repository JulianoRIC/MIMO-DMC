...
u_livre1 = zeros(1, 307);
u_livre2 = zeros(1, 196);
u_livre3 = zeros(1, 31);
u_livre4 = zeros(1, 44);
u_livre5 = zeros(1, 85);
u_livre6 = zeros(1, 19);
u_livre7 = zeros(1, 47);
u_livre8 = zeros(1, 125);

u_livre = [u_livre1 u_livre2 u_livre3 u_livre4 u_livre5 u_livre6 u_livre7 u_livre8];
...
free8 =zeros(1,101);
 ...
 for k = 1:305
free1(k) = y1(i) +  H(1,:)*u_livre';
 end     
 
 for k = 1:44
 free2(k) = y2(i) +  H(2,:)*u_livre';
 end
 
 for k = 1:116
 free3(k) = y3(i) +  H(3,:)*u_livre';
 end
 
 for k = 1:180
 free4(k) = y4(i) +  H(4,:)*u_livre';
 end
 
 for k = 1:53
 free5(k) = y5(i) +  H(5,:)*u_livre';
 end
 
 for k = 1:14
 free6(k) = y6(i) +  H(6,:)*u_livre';
 end
 
 for k = 1:124
 free7(k) = y7(i) +  H(7,:)*u_livre';
 end
 
 for k = 1:101
 free8(k) = y8(i) +  H(8,:)*u_livre';
 end

%considera ref futura cte
ref1 =w(i)*ones(1,N2(1))';
ref2 =w(i)*ones(1,N2(2))';
ref3 =w(i)*ones(1,N2(3))';
ref4 =w(i)*ones(1,N2(4))';
ref5 =w(i)*ones(1,N2(5))';
ref6 =w(i)*ones(1,N2(6))';
ref7 =w(i)*ones(1,N2(7))';
ref8 =w(i)*ones(1,N2(8))';

%calcula o controle 
deltaU1 = K1(1,:)*(ref1-free1'); %deltaU=k(w-f)
deltaU2 = K1(2,:)*(ref2-free2'); 
deltaU3 = K1(3,:)*(ref3-free3'); 
deltaU4 = K1(4,:)*(ref4-free4'); 
deltaU5 = K1(5,:)*(ref5-free5'); 
deltaU6 = K1(6,:)*(ref6-free6'); 
deltaU7 = K1(7,:)*(ref7-free7'); 
deltaU8 = K1(8,:)*(ref8-free8'); 
